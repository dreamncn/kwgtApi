...
class Clean
{
    ...
    static public function rewrite()
    {
        ...
        if ($data !== null && isset($data['real']) && isset($data['route'])) {
            if (isDebug()) {
                Log::info('route','Find Rewrite Cache: ' . $url . ' => ' . $data['real']);
            }
            $route_arr_cp = $data['route'];

        } else {
            if (isDebug()) {
                Log::info('route','Not Find Rewrite Cache: ' . $url);
            }
            $route_arr = [];
            ...
            if (isset($route_arr['m'])) {
                ...
                $__module = strtolower($route_arr['m']);
                unset($route_arr['m']);
            }
            $__controller = 'main';
            if (isset($route_arr['c'])) {

                $__controller = strtolower($route_arr['c']);
                unset($route_arr['c']);
            }
            $__action = 'index';
            if (isset($route_arr['a'])) {

                $__action = strtolower($route_arr['a']);
                unset($route_arr['a']);
            }
